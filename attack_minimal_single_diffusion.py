...
if __name__ == "__main__":
    ...
    directory = f'paper_results/{model_name}/{patch_mode}'
    ...
    if patch_mode =='timeout' and (isinstance(args.timeout, int) or isinstance(args.timeout, float)):
        ...
        directory = Path(f'paper_results/{model_name}/timeout_{args.timeout}Hz')
    else:
        timeout = None
    ...
    if args.model == 'frontnet':
        ...
        img_size = (96, 160)  # Height, Width
    elif args.model == 'yolov5':
        initial_patch_size = (150, 320)  # Height, Width
        img_size = (320, 640)  # Height, Width

    # if patch_mode == 'velo' or patch_mode == 'timeout' or patch_mode == 'random' or patch_mode == 'black' or patch_mode == 'white':
    controller = PController(max_vel=1.0, max_yaw_rate=0.2, kp_pos=1.0, kp_yaw=2.0)
    dt = 1/30
    ...
    if args.patch_mode == 'diffusion':
        ...
        diffusion_model = DiffusionModel(device=device, patch_size=initial_patch_size, prediction_model_name='frontnet')
        diffusion_model.load(f"overfit_results/diffusion_model_{model_name}_1000.pth")
        ...
        # # 3. Generate Patch
        # patch = diffusion_model.sample(n_samples=1, targets=new_cond, device=device, n_steps=50)
    ...
    while target_idx < len(target_trajectory) - 1:
        ...
        distance = torch.dist(cur_pos, target[:3], p=2)
        if distance > 1.5:
                print("Too far from target, skipping to next target.")
                target_idx += 1
                retries = 0
                optim_steps += 1

            # tgt_pos = torch.from_numpy(target[:3]).to(device)
        if distance > 0.05 and retries <= max_retries:
            print("Current position:", cur_pos)
            print("Target position:", target[:3])
            print("Distance to target:", distance.item())
    ...
    #         continue
        ...
        if patch_mode == 'black':
            ...
            # print("Raw tx, ty:", tx.item(), ty.item())
            ...
            if patch_mode == 'interpolation':
                ...
                with torch.no_grad():
                    patch = diffusion_model.sample(n_samples=1, targets=conditioning.unsqueeze(0), device=device, n_steps=100)
        else:
        ...
        if patch_mode == 'optimal' or patch_mode == 'timeout' or patch_mode == 'velo':
            ...
            while distance > 0.05 and i < 3000:
                ...
                if (T[0, 0].item() > 0.1) and (T[0, 2].item() > -10.) and (T[1, 2].item() > -10.) and (T[0, 2].item() < img_size[1]) and (T[1, 2].item() < img_size[0]):  # avoid too small patches
                ...
                if model_name == 'frontnet':
                    ...
                    # print("T_pred_in_drone within loop:", T_pred_in_drone)
                    # print("T_drone_in_world within loop:", T_drone_in_world)
                    ...
                    # print("Target box for 160x96 image: ", scaled_box)
                    # plt.imshow(manipulated_image_resized[0].permute(1,2,0).detach().cpu().numpy())
                    # plt.scatter([scaled_box[0,0].item(), scaled_box[0,2].item()], [scaled_box[0,1].item(), scaled_box[0,3].item()], c='g')
                    # plt.scatter([(scaled_box[0,0] + scaled_box[0,2]).item() / 2], [(scaled_box[0,1] + scaled_box[0,3]).item() / 2], c='r')
                    # plt.savefig(f'debugging_yolo_box.png')
                    # plt.close()
                    ...
                    T_rec_pred_in_drone = T_matrix(recovered_xyzyaw[0])
                    T_rec_pred_in_world = T_drone_in_world @ T_rec_pred_in_drone
                    # print("T_rec_pred_in_world within loop:", T_rec_pred_in_world)
                    ...
                    delta_vector = T_rec_pred_in_world[:3, 3] - T_drone_in_world[:3, 3]
                    setpoint_yaw = torch.atan2(delta_vector[1], delta_vector[0]) * -1.
                    # print("Calculated yaw to patch box within loop:", setpoint_yaw.item())
                    T_direction_world = torch.eye(4, device=device, dtype=torch.float32)
                    T_direction_world[:3, 3] = calc_heading_vec(1., normalize_yaw_t(setpoint_yaw - torch.pi)).to(device)
                    # print("Direction in world within loop:", T_direction_world)
                    ...
                    # print("T_setpoint_world within loop:", T_setpoint_world)
                    # print("Current drone pose in world:", T_drone_in_world)
                    # print("Target in world:", target[:3], target[3].item())
                    # calculate yaw such that the drone faces the monitor
                    # yaw = torch.atan2(T_drone_in_world[1, 3] - T_setpoint_world[1, 3], T_drone_in_world[0, 3] - T_setpoint_world[0, 3])
                    # print("Calculated yaw to patch box:", yaw.item())
                    # yaw = torch.atan2(T_setpoint_world[1, 0], T_setpoint_world[0, 0])
                    # setpoint_yaw = normalize_yaw_t(yaw)
                    # setpoint_yaw = normalize_yaw_t(target_yaw)

                    prediction = torch.stack([*T_setpoint_world[:3, 3], setpoint_yaw]).to(device) # prediction values
                    # print("Predicted setpoint in world: ", T_setpoint_world[:3, 3], setpoint_yaw.item())
                    distance = torch.dist(prediction[:3], target[:3], p=2)
                    angular_loss = 1 - torch.cos(normalize_yaw_t(prediction[3]) - normalize_yaw_t(target[3]))
                    ...
                    # with torch.no_grad():
                            ...
                            # setpoint_yaw = prediction[3]
                            # prediction = torch.stack([*T_setpoint_world[:3, 3], setpoint_yaw]).to(device)
                ...
                if i % 100 == 0:
                    ...
                    # print("Predicted yaw: ", prediction[3].item(), "Target yaw:", target[3].item())

                patch.data.clamp_(0., 1.)
                i += 1

            # print("Output dir: ", output_dir)
            print("Predicted setpoint: ", best_setpoint)
            ...
            print("Patch mode:", patch_mode)
            print("Patch size:", patch.shape)
            print("T matrix:", T)
            print("Manipulated image min/max:", manipulated_image.min().item(), manipulated_image.max().item())
            ...
            if model_name == 'frontnet':
                ...
                prediction = prediction.squeeze(2).mT
                    ...
                    prediction = torch.stack([*T_setpoint_world[:3, 3], setpoint_yaw]).to(device).unsqueeze(0) # prediction values
            ...
            print("Predicted setpoint in world after control: ", T_setpoint_world[:3, 3], prediction[3].item())
            
            # print("Prediction before appending to all_drone_poses:", prediction)
            best_setpoint = prediction.detach().clone()
        ...
        # last_setpoint = best_setpoint.clone().detach()
