        ...
        if ret:
            ...
            for detected_person in range(nms_boxes.shape[0]):
                ...
                if not person_dictionary:
                    person_dictionary[detected_person] = deque([person_features], maxlen=20)
                    print(f"New person added: {len(person_dictionary)}")
                    detected_person_idx = 0
                elif len(person_dictionary) < nms_boxes.shape[0]:
                    person_dictionary[len(person_dictionary)] = deque([person_features], maxlen=20)
                    print(f"New person added: {len(person_dictionary)}")
                    ...
                    similarity_matrix = np.zeros((len(person_dictionary), 20))
                    ...
                    if highest_similarity < 0.2: # that's the threshold they use here: https://github.com/samihormi/Multi-Camera-Person-Tracking-and-Re-Identification/blob/main/demo.py#L70
                        ...
                        person_dictionary[num_persons] = deque([person_features], maxlen=20)
