...
apptainer run --nv /home/piha/container.sif python attack_minimal_single.py -t "${TRAJ}" --display_size "${DISPLAY_SIZE}" --seed "${SEED}" --mode "${PIC_MODE}" --img_idx "${IMG_IDX}" --timeout "${TIMEOUT}"
EOF
}
