...
def corners2transformation(ul, ur, ll, lr, center, projector_matrix):
...
class PatchDisplayThread(Thread):
    ...
    def update(self, patch, sf, tx, ty, drone_pose):
        # Add the image to the queue
        T = self.bb_opt2transformation(sf, tx, ty, drone_pose)
    ...
    def bb_opt2transformation(self, sf_opt, tx_opt, ty_opt, drone_pose, patch_size=80):
        ...
        T = corners2transformation(intersection_ul, intersection_ur, intersection_ll, intersection_lr, intersection_center, self.projector_matrix)
        return T
